...
*/
void EmuApi::EmuWinMain(uc_engine* uc, DWORD_PTR address, size_t size, void* user_data)
{
    ...
    uc_reg_read(uc, UC_X86_REG_RDX, &hPrevInstance);

    LPSTR lpCmdLine{};
    uc_reg_read(uc, UC_X86_REG_R8, &lpCmdLine);

    std::string rlalpCmdLine;
    if(!EmuReadNullTermString(uc, (DWORD_PTR)lpCmdLine, rlalpCmdLine)) { printf("Error when read lpCmdLine in WinMain"); _CrtDbgBreak(); }

    int nShowCmd{};
    uc_reg_read(uc, UC_X86_REG_R9D, &nShowCmd);


    int WinMainResult = WinMain((HINSTANCE)hInstance,(HINSTANCE)hPrevInstance,(LPSTR)rlalpCmdLine.data(),(int)nShowCmd);


    uc_reg_write(uc, UC_X86_REG_EAX, &WinMainResult);
    uc_reg_write(uc, UC_X86_REG_RCX, &hInstance);
    uc_reg_write(uc, UC_X86_REG_RDX, &hPrevInstance);
    if(!EmuWriteNullTermString(uc, (DWORD_PTR)lpCmdLine, rlalpCmdLine)) { printf("Error when read lpCmdLine in WinMain"); _CrtDbgBreak(); }
    uc_reg_write(uc, UC_X86_REG_R9D, &nShowCmd);
...
*/
void EmuApi::EmuwWinMain(uc_engine* uc, DWORD_PTR address, size_t size, void* user_data)
{
    ...
    std::wstring rlwlpCmdLine;
    if(!EmuReadNullTermUnicodeString(uc, (DWORD_PTR)lpCmdLine, rlwlpCmdLine)) { printf("Error when read lpCmdLine in wWinMain"); _CrtDbgBreak(); }

    int nShowCmd{};
    uc_reg_read(uc, UC_X86_REG_R9D, &nShowCmd);


    int wWinMainResult = wWinMain((HINSTANCE)hInstance,(HINSTANCE)hPrevInstance,(LPWSTR)rlwlpCmdLine.data(),(int)nShowCmd);


    uc_reg_write(uc, UC_X86_REG_EAX, &wWinMainResult);
    uc_reg_write(uc, UC_X86_REG_RCX, &hInstance);
    uc_reg_write(uc, UC_X86_REG_RDX, &hPrevInstance);
    if(!EmuWriteNullTermUnicodeString(uc, (DWORD_PTR)lpCmdLine, rlwlpCmdLine)) { printf("Error when read lpCmdLine in wWinMain"); _CrtDbgBreak(); }
    uc_reg_write(uc, UC_X86_REG_R9D, &nShowCmd);
    #ifdef TRACE_LOG_CONSOLE
    *outs << "EmuwWinMain\n";
    #endif
...
DWORD
__cdecl
WTSGetServiceSessionId(VOID);

*/
void EmuApi::EmuWTSGetServiceSessionId(uc_engine* uc, DWORD_PTR address, size_t size, void* user_data)
{


    DWORD WTSGetServiceSessionIdResult = WTSGetServiceSessionId();


    uc_reg_write(uc, UC_X86_REG_EAX, &WTSGetServiceSessionIdResult);
    #ifdef TRACE_LOG_CONSOLE
    *outs << "EmuWTSGetServiceSessionId\n";
    #endif
...
BOOLEAN
__cdecl
WTSIsServerContainer(VOID);

*/
void EmuApi::EmuWTSIsServerContainer(uc_engine* uc, DWORD_PTR address, size_t size, void* user_data)
{


    BOOLEAN WTSIsServerContainerResult = WTSIsServerContainer();


    uc_reg_write(uc, UC_X86_REG_AL, &WTSIsServerContainerResult);
    #ifdef TRACE_LOG_CONSOLE
    *outs << "EmuWTSIsServerContainer\n";
    #endif
